 ...
 *  Copylefts (C) 2007-2024  Krzysztof Ciba
 *
...
class dcube_header {
   ...
   */
  function __construct( $project, $version, $author, $css, $js, $where ) {
...
class dcube_body {
	 ...
	 * @param $where relative path to dcube.php file
	 */
	function __construct( $project, $version, $author, $xml_file, $pDir, $log_file, $lDir, $where) {
	  ...
	  echo "<div class='lheader'><a href='dcube.html'>[ Go to HTML display ]</a></div>\n";
	  echo "<div class='header'><b>".$version."</b>".$project." (<b>PHP</b>) by ".
	...
	function footer() {
	  ...
	  echo "<a href='https://twiki.cern.ch/twiki/bin/view/AtlasComputing/DCube'>[&nbsp;Documentation&nbsp;]</a>\n";
	...
	function mainPanel( ) {
	  ...
	  // 1st button
	  echo "<div id='t1' class='tabpane sel_tp' onClick='selPane(\"1\");'>";
	...
	function treeView( $dirNode, $level=0 ) {
		...
		echo 	" title='".$dirName."' onmouseover=\"statusBar('TDirectory ".$dirName."');\"".
	...
	function histContentsDIV( $histNode, $id ) {
		...
		if ( strlen($status) ) {
			...
			foreach ( $imgNodes as $key => $val ) {
				...
				if ( strstr($type, "reg") ) {
					echo "<div id='".$id."' class='tabpane sel_tp' onClick='selPlot(\"".$id."\", \"".$src."\");'>";
					$type = "Normal";
				} else {
...
class dcube {
   ...
   */
  function __construct( $xml_file, $log_file = NULL, $whereiam = "" ) {
